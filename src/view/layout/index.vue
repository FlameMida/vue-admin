<template>
  <el-container class="layout-cont">
    <el-container :class="[isSlider?'openside':'hideside',isMobile ? 'mobile': '']">
      <el-row :class="[isShadowBg?'shadowBg':'']" @click="changeShadow()" />
      <el-aside class="main-cont main-left">
        <div class="tilte" :style="{background: backgroundColor}">
          <img alt class="logoimg" :src="$VUE_ADMIN.appLogo">
          <h2 v-if="isSlider" class="tit-text" :style="{color:textColor}">{{ $VUE_ADMIN.appName }}</h2>
        </div>
        <Aside class="aside" />
      </el-aside>
      <!-- 分块滑动功能 -->
